# Area de Apoio

## Bem-vindo 👋

Olá,
Bem-Vindo à Área de apoio da **aplicação de Gestão de Recessão**.

Esta área foi criada para esclarecer dúvidas especificas, obter apoio técnico e requisitar melhorias na aplicação .

## Como podemos ajudar

Na area de apoio podemos encontrar:

- Esclarecimento de duvidas sobre reservas, quartos, clientes e horário

- Apoio em caso de erros ou dificuldades técnicas

- Informações sobre atualização da aplicação

## Contacto do suporte

- 📷Email: suporte@gmail.com

- 📞Telefone: +358 405 930 170

- ⏱️Horário: Segunda a Sexta-Feira das 9:00h ás 18:00h

## Problemas comuns

Alguns problemas mais frequentes:

- Erro ao inserir ou atualizar dados

- Digitar dados incorretos(Numero,Texto,Decimal)

- Tentativa de apagar algum registo que não está lá

## Sugestões e Melhorias

A tua opinião é muito importante para nós.
Se tiver alguma sugestão para melhoria da aplicação, envie-nos uma mensagem para o email de suporte.

## Agradecimento

Obrigado por utilizar a **aplicação de recessão do hotel**.
Estamos sempre disponiveis para ajudar e qualquer sugestão e Melhorias.